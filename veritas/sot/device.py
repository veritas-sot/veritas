...
class Device:
    ...
    def update_interface(self, properties):
        ...
        if not interface:
            logging.error(f'unknown interface {self._interface} on {self._device}')
            return False

        try:
            return interface.update(properties)
        except Exception as exc:
            logging.error(f'could not update interface; got exception {exc}')
            return False
    ...
    def delete_tags(self, tags_to_delete):
        ...
        logging.debug(f'deleting tags {tags_to_delete} on {self._device}')

        # the device must exist; get tags
        device = self._nautobot.dcim.devices.get(name=self._device)
        if device is None:
            logging.error(f'unknown device {self._device}')
            return None
        ...
        logging.debug(f'new tags {device_tags}')
    ...
    def delete_interface_tags(self, tags_to_delete):
        logging.debug(f'deleting tags {tags_to_delete} on {self._device}/{self._interface}')
        ...
        logging.debug(f'current tags: {current_tags}')
        logging.debug(f'new tags {interface_tags}')

        properties = {'tags': interface_tags}
        try:
            return interface.update(properties)
        except Exception as exc:
            logging.error(f'could not delete tags on interface; got exception {exc}')
            return False
